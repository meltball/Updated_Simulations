...
# ============================================================
# REALISTIC FEATURE DETECTION POPUPS
# ============================================================

class FeatureDetectionSystem:
    """
    Creates semi-realistic detection boxes and popups.
    These are visual callouts only, but they mimic what a real AI detection UI would show:
    - feature label
    - confidence score
    - bounding box
    - pulse marker
    ...
    def __init__(self, width: int, height: int):
        ...
        self.features = [
            {"label": "MINERAL VEIN", "x": 0.23, "y": 0.34, "w": 0.18, "h": 0.10, "conf": 0.91, "phase": 0.0},
            {"label": "THERMAL SIGNATURE", "x": 0.64, "y": 0.28, "w": 0.16, "h": 0.12, "conf": 0.87, "phase": 1.2},
            {"label": "ANOMALY CLUSTER", "x": 0.47, "y": 0.61, "w": 0.22, "h": 0.14, "conf": 0.94, "phase": 2.1},
            {"label": "MOTION TRAIL", "x": 0.70, "y": 0.68, "w": 0.17, "h": 0.09, "conf": 0.82, "phase": 3.0},
            {"label": "HIGH-DENSITY ZONE", "x": 0.18, "y": 0.73, "w": 0.20, "h": 0.13, "conf": 0.89, "phase": 4.4},
            {"label": "EDGE FEATURE", "x": 0.55, "y": 0.45, "w": 0.13, "h": 0.08, "conf": 0.78, "phase": 5.5},
        ]

    def update(self, t: float, state: FractalState):
        # Small movement makes boxes look tracked rather than static.
        for i, f in enumerate(self.features):
            f["dx"] = 0.012 * math.sin(t * 0.8 + f["phase"])
            f["dy"] = 0.010 * math.cos(t * 0.7 + f["phase"])
            f["live_conf"] = max(
                0.65,
                min(0.99, f["conf"] + 0.045 * math.sin(t * 1.4 + f["phase"]))
            )
    ...
    def draw(self, ax, t: float):
        ...
        for i, f in enumerate(self.features):
            ...
            alpha = 0.55 + 0.35 * max(0.0, math.sin(t * 2.0 + f["phase"]))
            ...
            # Label popup
            label = f"ID-{i+101}  {f['label']}  {f.get('live_conf', f['conf']):.0%}"
            txt = ax.text(
                x,
                max(8, y - 10),
                label,
            ...
            pulse_size = 45 + 30 * max(0, math.sin(t * 3.0 + f["phase"]))
            pulse = ax.scatter([cx], [cy], s=pulse_size, facecolors="none", edgecolors="white", alpha=0.25)
...
def run_demo():
    ...
    def update(_frame):
        ...
        detections.update(engine.time, state)
